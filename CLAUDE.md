...
1. **Claude 先建模板**：`素材/<年份>.txt`（本地、gitignore，handoff 草稿，**内容留空、不加 `#` 头部说明**，与既有各年 txt 样式保持一致）；并建好照片目录 `public/photos/<年份>/`，内放一个空 **`.gitkeep`**（保持与既有各年份目录一致，让空目录能进版本控制；图片由用户放进该目录，会提交、随构建部署）。
